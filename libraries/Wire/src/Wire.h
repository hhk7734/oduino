...
public:
    ...
    void beginTransmission( int slave_address ) { beginTransmission( ( uint8_t )slave_address ); }
    uint8_t endTransmission( uint8_t stop );
    uint8_t endTransmission( void ) { return endTransmission( true ); }
    uint8_t requestFrom( uint8_t slave_address, uint8_t len )
    {
        return requestFrom( slave_address, len, ( uint8_t ) true );
    }
    uint8_t requestFrom( uint8_t slave_address, uint8_t len, uint8_t stop )
    {
        return requestFrom( slave_address, len, 0, 0, stop );
    }
    uint8_t requestFrom( uint8_t  slave_address,
                         uint8_t  len,
                         uint32_t reg_address,
                         uint8_t  reg_address_size,
                         uint8_t  stop );
    uint8_t requestFrom( int slave_address, int len )
    {
        return requestFrom( ( uint8_t )slave_address, ( uint8_t )len );
    }
    uint8_t requestFrom( int slave_address, int len, int stop )
    {
        return requestFrom( ( uint8_t )slave_address, ( uint8_t )len, ( uint8_t )stop );
    }
