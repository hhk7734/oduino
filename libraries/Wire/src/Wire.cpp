...
void TwoWire::end( void ) { close( fd ); }
...
uint8_t TwoWire::requestFrom( uint8_t  slave_address,
                              uint8_t  len,
...
{
    if( tx_buff_index < I2C_BUFFER_SIZE ) { tx_buff[tx_buff_index++] = data; }
    else
    {
    }
}
...
{
    if( rx_buff_index < rx_buff_len ) { return rx_buff[rx_buff_index++]; }
    else
    {
        return 0;
    }
