...
//
/*----------------------------------------------------------------------------*/
static int gpioToShiftReg( int pin ) { return pin % 32; }
...
//
/*----------------------------------------------------------------------------*/
static int gpioToShiftGReg( int pin ) { return pin % 8; }

/*----------------------------------------------------------------------------*/
static int _getModeToGpio( int mode, int pin )
{
    if( pin > 255 ) return msg( MSG_ERR, "%s : Invalid pin number %d\n", __func__, pin );
...
static int _getAlt( int pin )
{
    ...
    if( ret == 0 )
        ret = *( gpio[bank] + ( N1_GPIO_CON_OFFSET >> 2 ) ) & ( 1 << gpioToShiftReg( pin ) ) ? 1
                                                                                             : 0;
    else
    {
...
static int _digitalRead( int pin )
{
    ...
    ret = *( gpio[bank] + ( N1_GPIO_GET_OFFSET >> 2 ) ) & ( 1 << gpioToShiftReg( pin ) ) ? HIGH
                                                                                         : LOW;
        ...
        if( lib->sysFds[pin] != -1 )
        {
            if( value == LOW )
            {
                if( write( lib->sysFds[pin], "0\n", 2 ) < 0 )
                    msg( MSG_ERR,
                         "%s : %s\nEdit direction file to output mode "
...
static unsigned int _digitalReadByte( void )
{
    ...
    /* Wiring PI GPIO0 = N1 GPIO1_A.1 */
    if( gpioBits1.bits.bit1 ) value |= 0x01;
    /* Wiring PI GPIO1 = N1 GPIO1_A.2 */
    if( gpioBits1.bits.bit2 ) value |= 0x02;
    /* Wiring PI GPIO2 = N1 GPIO1_A.3 */
    if( gpioBits1.bits.bit3 ) value |= 0x04;
    /* Wiring PI GPIO3 = N1 GPIO1_A.4 */
    if( gpioBits1.bits.bit4 ) value |= 0x08;
    /* Wiring PI GPIO4 = N1 GPIO1_B.5 */
    if( gpioBits1.bits.bit14 ) value |= 0x10;
    /* Wiring PI GPIO5 = N1 GPIO1_C.2 */
    if( gpioBits1.bits.bit19 ) value |= 0x20;
    /* Wiring PI GPIO6 = N1 GPIO1_D.0 */
    if( gpioBits1.bits.bit25 ) value |= 0x40;
    /* Wiring PI GPIO7 = N1 GPIO1_A.0 */
    if( gpioBits1.bits.bit0 ) value |= 0x80;
...
static void init_gpio_mmap( void )
{
    ...
    cru[0] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                 N1_PMUCRU_BASE );

    // GPIO{2, 3, 4}
    //#define ODROIDN1_CRU_BASE	0xFF760000
    cru[1]
        = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, N1_CRU_BASE );

    // GPIO{0, 1}
    //#define ODROIDN1_PMU_BASE	0xFF320000
    grf[0] = ( uint32_t * )mmap( 0, N1_GRF_BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                 N1_PMUGRF_BASE );

    // GPIO{2, 3, 4}
    //#define ODROIDN1_GRF_BASE	0xFF770000
    grf[1] = ( uint32_t * )mmap( 0, N1_GRF_BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                 N1_GRF_BASE );
    ...
    gpio[1] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                  N1_GPIO_1_BASE );

    // GPIO2_C.	0_B,1_B
    //#define ODROIDN1_GPIO2_BASE	0xFF780000
    gpio[2] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                  N1_GPIO_2_BASE );

    // GPIO4_C.	5,6
    // GPIO4_D.	0,4,5,6
    //#define ODROIDN1_GPIO4_BASE	0xFF790000
    gpio[4] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                  N1_GPIO_4_BASE );

    // Reserved
    gpio[0] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                  N1_GPIO_0_BASE );
    gpio[3] = ( uint32_t * )mmap( 0, BLOCK_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd,
                                  N1_GPIO_3_BASE );

    if( ( ( int32_t )cru[0] == -1 ) || ( ( int32_t )cru[1] == -1 ) )
    { return msg( MSG_ERR, "wiringPiSetup: mmap (CRU) failed: %s\n", strerror( errno ) ); }

    if( ( ( int32_t )grf[0] == -1 ) || ( ( int32_t )grf[1] == -1 ) )
    { return msg( MSG_ERR, "wiringPiSetup: mmap (GRF) failed: %s\n", strerror( errno ) ); }

    if( ( ( int32_t )gpio[0] == -1 ) || ( ( int32_t )gpio[1] == -1 ) || ( ( int32_t )gpio[2] == -1 )
        || ( ( int32_t )gpio[3] == -1 ) || ( ( int32_t )gpio[4] == -1 ) )
