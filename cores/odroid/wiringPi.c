...
const char *piModelNames[16] = {
    "Unknown", "ODROID-C1/C1+", "ODROID-C2", "ODROID-XU3/4", "ODROID-N1", "ODROID-N2",
};
...
int moduleLoaded( char *modName )
{
    ...
    {
        if( strncmp( line, modName, len ) != 0 ) continue;
...
int msg( int type, const char *message, ... )
{
    ...
    if( type != MSG_WARN ) exit( EXIT_FAILURE );
    return 0;
}
...
static void digitalWrite8Dummy( UNU struct wiringPiNodeStruct *node, UNU int pin, UNU int value )
{
    return;
}
static int  digitalReadDummy( UNU struct wiringPiNodeStruct *node, UNU int UNU pin ) { return LOW; }
static void digitalWriteDummy( UNU struct wiringPiNodeStruct *node, UNU int pin, UNU int value )
{
    return;
}
    ...
    for( pin = pinBase; pin < ( pinBase + numPins ); ++pin )
        if( wiringPiFindNode( pin ) != NULL )
            ( void )wiringPiFailure(
                WPI_FATAL, "wiringPiNewNode: Pin %d overlaps with existing definition\n", pin );
...
int piGpioLayout( void )
{
    ...
    if( gpioLayout != -1 ) return gpioLayout;
    ...
    if( strncmp( line, "Hardware", 8 ) != 0 ) wiringPiFailure( WPI_FATAL, "No \"Hardware\" line" );
    ...
    if( strncmp( line, "Revision", 8 ) != 0 ) wiringPiFailure( WPI_FATAL, "No \"Revision\" line" );
    ...
    if( *c != ':' ) wiringPiFailure( WPI_FATAL, "Bogus \"Revision\" line (no colon)" );
    ...
    if( strlen( c ) < 4 ) wiringPiFailure( WPI_FATAL, "Bogus revision line (too small)" );
    ...
    if( ( strcmp( c, "0002" ) == 0 ) || ( strcmp( c, "0003" ) == 0 ) || ( strcmp( c, "000a" ) == 0 )
        || ( strcmp( c, "0100" ) == 0 ) || ( strcmp( c, "0400" ) == 0 ) )
        gpioLayout = 1;
    else
        gpioLayout = 2;
    ...
    if( wiringPiDebug ) printf( "BoardRev: Returning revision: %d\n", libwiring.rev );

    return libwiring.rev;
}

/*----------------------------------------------------------------------------*/
int piBoardRev( void ) { return piGpioLayout(); }
...
{
    if( libwiring.getModeToGpio ) return libwiring.getModeToGpio( MODE_PINS, wpiPin );
...
{
    if( libwiring.getModeToGpio ) return libwiring.getModeToGpio( MODE_PHYS, physPin );
...
{
    if( libwiring.setPadDrive ) return libwiring.setPadDrive( pin, value );
...
{
    if( libwiring.getPadDrive ) return libwiring.getPadDrive( pin );
...
int getAlt( int pin )
{
    if( libwiring.getAlt ) return libwiring.getAlt( pin );
...
int getPUPD( int pin )
{
    if( libwiring.getPUPD ) return libwiring.getPUPD( pin );
...
{
    if( libwiring.pinMode ) return libwiring.pinMode( pin, mode );
}

/*----------------------------------------------------------------------------*/
void pullUpDnControl( int pin, int pud )
{
    if( libwiring.pullUpDnControl ) return libwiring.pullUpDnControl( pin, pud );
}

/*----------------------------------------------------------------------------*/
int digitalRead( int pin )
{
    if( libwiring.digitalRead ) return libwiring.digitalRead( pin );
...
{
    if( libwiring.digitalWrite ) return libwiring.digitalWrite( pin, value );
}

/*----------------------------------------------------------------------------*/
int analogRead( int pin )
{
    if( libwiring.analogRead ) return libwiring.analogRead( pin );
...
{
    if( libwiring.digitalWriteByte ) return libwiring.digitalWriteByte( value );
}

/*----------------------------------------------------------------------------*/
unsigned int digitalReadByte( void )
{
    if( libwiring.digitalReadByte ) return libwiring.digitalReadByte();
...
int waitForInterrupt( int pin, int mS )
{
    ...
    if( ( fd = libwiring.sysFds[PIN_NUM_CALC_SYSFD( pin )] ) == -1 ) return -2;
...
int wiringPiISR( int pin, int mode, void ( *function )( void ) )
{
    ...
    if( libwiring.mode == MODE_UNINITIALISED )
        return wiringPiFailure( WPI_FATAL, "wiringPiISR: wiringPi has not been initialised. "
                                           "Unable to continue.\n" );

    if( libwiring.getModeToGpio )
        GpioPin = libwiring.getModeToGpio( libwiring.mode, pin );
    else
        return wiringPiFailure( WPI_FATAL, "%s: getModeToGpio function not initialize!\n",
                                __func__ );
...
int wiringPiSetup( void )
{
    ...
    // sysFds init
    for( i = 0; i < 256; i++ ) libwiring.sysFds[i] = -1;
    // init wiringPi mode
    ...
    {
        case MODEL_ODROID_C1: init_odroidc1( &libwiring ); break;
        case MODEL_ODROID_C2: init_odroidc2( &libwiring ); break;
        case MODEL_ODROID_XU3: init_odroidxu3( &libwiring ); break;
        case MODEL_ODROID_N1: init_odroidn1( &libwiring ); break;
        case MODEL_ODROID_N2: init_odroidn2( &libwiring ); break;
        default: return wiringPiFailure( WPI_ALMOST, "wiringPiSetup: Unknown model\n" );
    }
    ...
    for( pin = 0; pin < 256; ++pin )
    {
        switch( libwiring.model )
        {
            ...
            case MODEL_ODROID_N2:
                ...
                break;
            default: sprintf( fName, "/sys/class/gpio/gpio%d/value", pin ); break;
        }
